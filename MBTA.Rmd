...
may_calendar_dates <- fread(may_listfile[3]) # useful for service modification
may_calendar <- fread(may_listfile[4]) # unique service_id
may_directions<- fread(may_listfile[6])
may_lines <- fread(may_listfile[12])
may_routes <- fread(may_listfile[17])
may_shapes <- fread(may_listfile[18]) # draw points
may_stop_times <- fread(may_listfile[19])
may_stops <- fread(may_listfile[20])
may_transfers <- fread(may_listfile[21]) # has transfer_time info
may_trips<- fread(may_listfile[22])
...
Headway is the actual time between the trip and the previous trip at the stop, in seconds. This can be used for checking the reliability of bus service at a given stop for a trip. Since the headway can be only evaluated when using the headway standard, the missing values of headway caused by schedule standard are removed. I found silver line bus generally has more route stop pairs, thus, I narrow down to draw the distribution of the silver lines in May on for exploration.

```{r messgage = FALSE, warning=FALSE,echo=FALSE, fig.cap="Figure 1. Silver Line bus travel times", fig.show='hold', out.width="50%"}
...
bus05 <- fread(listfile_bus[7]) %>%  filter(between(service_date, as.Date("2022-05-14"), as.Date("2022-05-21"))) %>% filter(standard_type == "Headway")
...
# Year 2022
HR_2022_Q1 <- fread(listfile_rt[1])
LR_2022_Q1 <- fread(listfile_rt[2])
HR_2022_Q2 <- fread(listfile_rt[3])
LR_2022_Q2 <- fread(listfile_rt[4])
HR_2022_Q3 <- fread(listfile_rt[5])
LR_2022_Q3 <- fread(listfile_rt[6])

# Year 2021
HR_2021_Q1 <- fread(listfile_rt[7])
LR_2021_Q1 <- fread(listfile_rt[11])
HR_2021_Q2 <- fread(listfile_rt[8])
LR_2021_Q2 <- fread(listfile_rt[12])
HR_2021_Q3 <- fread(listfile_rt[9])
LR_2021_Q3 <- fread(listfile_rt[13])
HR_2021_Q4 <- fread(listfile_rt[10])
LR_2021_Q4 <- fread(listfile_rt[14])
...
p_red_routes<- plot_ly(x=~red_routes$travel_time_sec, name = "Red", type = "histogram") %>%
  layout( xaxis=list(range=c(300,3500)))
...
p_greenb_routes<- plot_ly(x=~greenb_routes$travel_time_sec, name = "Green-B", type = "histogram") %>%
  layout( xaxis=list(range=c(300,3500)))

p_greenc_routes<- plot_ly(x=~greenc_routes$travel_time_sec, name = "Green-C", type = "histogram") %>%
  layout( xaxis=list(range=c(300,3500)))

p_greend_routes<- plot_ly(x=~greend_routes$travel_time_sec, name = "Green-D", type = "histogram") %>%
  layout( xaxis=list(range=c(300,3500)))

p_greene_routes<- plot_ly(x=~greene_routes$travel_time_sec, name = "Green-E", type = "histogram") %>%
  layout( xaxis=list(range=c(300,3500)))

p_mattpan_routes<- plot_ly(x=~mattpan_routes$travel_time_sec, name = "Mattpan", type = "histogram") %>%
  layout(xaxis=list(range=c(300,3500)))

subplot(p_red_routes,p_blue_routes,p_orange_routes,p_greenb_routes, p_greenc_routes,p_greend_routes,p_greene_routes,p_mattpan_routes,nrows=4, shareY = T, titleY = T, shareX = T) %>% layout(title = " Travel time distribution by line", xaxis = list(title = "Travel time in seconds"), yaxis=list(title="Frequency"))
...
```{r tidy=TRUE, warning = FALSE, message = FALSE, fig.show='hold', out.width="50%",fig.cap="Figure 4. Red line travel time density plot"}
####RED LINE
...
```{r tidy=TRUE, warning = FALSE, message = FALSE, fig.cap=" Figure 5. Orange line travel time density plot", fig.show='hold', out.width="50%"}
##### Orange line
# match stop_lon and stop_lat
